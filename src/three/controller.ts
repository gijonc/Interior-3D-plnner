  ...
  export var Controller = function (three, model, camera, element, controls, hud) {
    ...
    function itemLoaded(item) {
      if (!item.position_set) {
		  var initLoad = true;
		  scope.setSelectedObject(item, initLoad);
		  // this makes cursor not onto the item when initially loaded
    ...
    function onEntry(state) {
      switch (state) {
        ...
        case states.DRAGGING:
          ...
          three.itemBeingDragged.fire(true); // fire out mouse position
          clickPressed();
          controls.enabled = false;
          break;
      }
    ...
    function onExit(state) {
      switch (state) {
        ...
        case states.SELECTED:
          break;
		  case states.DRAGGING:
			 three.itemBeingDragged.fire(false, mouse);
          if (mouseoverObject) {
    ...
    this.setSelectedObject = function (object, initLoad) {
      ...
      if (object != null) {
        ...
        three.itemSelectedCallbacks.fire(object, initLoad || mouse);
      } else {
        selectedObject = null;

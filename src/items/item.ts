...
module BP3D.Items {
  ...
  export abstract class Item extends THREE.Mesh {
     ...
     * @param scale TODO 
     */
    constructor(protected model: Model.Model, public metadata: Metadata, geometry: THREE.Geometry, material: THREE.Material[], position: THREE.Vector3, rotation: number, scale: THREE.Vector3) {
      super();
    ...
    public initObject = function () {
		...
		// An ugly hack to increase the size of gltf models
		 if (this.halfSize.x < 1.0) {
			 this.resize(this.getHeight() * 50, this.getWidth() * 50, this.getDepth() * 50);
		 }
      // select and stuff
      this.scene.needsUpdate = true;
    };
    ...
    public updateHighlight() {
      ...
      (<THREE.Material[]>this.material).forEach((material) => {
